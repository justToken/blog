...
function pagination($count,$perlogs,$page,$url,$anchor=''){
    ...
    #开始分页导航内容
	$re = '<div class="panel panel-warning"><div class="panel-body"><ul class="pagination">';
	if($pnums<=1) return false;	//如果只有一页则跳出	
	if($page!=1) $re .="<li><a href=\"$urlHome$anchor\">首页</a> </li>"; 
	if($prepg) $re .=" <li><a href=\"$url$prepg$anchor\">前页</a> </li>";
	for ($i = $page-5;$i <= $page+5 && $i <= $pnums; $i++){
		if ($i > 0){
			if ($i == $page){
				$re .= " <li class='disabled active'><a href='javascript:void(0);'>$i</a></li> ";
			} elseif($i == 1) {
	...
	if($nextpg) $re .=" <li><a href=\"$url$nextpg$anchor\">后页</a></li> "; 
	if($page!=$pnums) $re.="<li> <a href=\"$url$pnums$anchor\" title=\"尾页\">尾页</a></li>";
	
	
	$re .="</ul></div><div class='panel-footer center-block'>共{$pnums}页</div></div>";
	return $re;
}
...
/**
 * 获取Gravatar头像
 * http://gravatar.duoshuo.com/site/implement/images/
 * @param $email
 * @param $s size
...
<!-- qtnote.com Baidu tongji analytics -->
<script type="text/javascript">
var _bdhmProtocol = (("https:" == document.location.protocol) ? " https://" : " http://");
document.write(unescape("%3Cscript src='" + _bdhmProtocol + "hm.baidu.com/h.js%3F63cc4916e4bdb8d541d5b864e023cc36' type='text/javascript'%3E%3C/script%3E"));
</script>
</body>
</html>
EOT;
